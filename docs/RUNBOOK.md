...
The owner does five things you must never do: create the account, send the funds, derive the private key offline and paste it into the env file, sign the enrollment, and say go before each live tick. You never ask for a seed phrase or private key, never print one, never read an env file that holds one, and never set `EXECUTION_MODE=live` without an explicit go. If a key or seed ever appears in this chat, treat it as burned and tell the owner to move the funds to a fresh account.
...
The agent runs the commands. The owner does five things the agent must never do:

- Create the wallet account and read its address off the wallet.
- Send the funds.
- Derive the private key, offline, and paste it into the env file.
- Sign the enrollment message on the campaign page.
- Say the word before the first live tick, per pool.

The agent never asks for a private key or seed phrase, never prints one, never reads an env file that holds one, and never sets `EXECUTION_MODE=live` without that explicit go. A key that lands in chat, a note or a screenshot is treated as burned: move the funds to a fresh account and restart from the address step.
...
| --- | --- | --- |
| The owner | The seed phrase and every private key, the funds and their amounts, the enrollment signature, the go before each live tick and each Manual CLI swap | Pastes a key or seed into chat |
| The agent | The order of steps, the config writes (address, kill-switch path, mode), every check, starting and stopping the loop, reading results back in plain words | Sees, prints or derives a key; sets `EXECUTION_MODE=live` without the go; changes the bot's strategy or numbers; runs a Manual CLI swap unasked; creates a wallet; drives the wallet or browser |
...
Hand-offs, in order: the agent installs and configures; hands the key step to the owner at Step 5; hands enrollment to the owner; on the owner's go, hands execution to the bot process. From then on the agent reads the bot and does not steer it. Strategy knobs (`F_STAR`, `CURVE_*`, `MAX_POSITION_USTX` and the rest) are the owner's decision, applied by editing the env file and restarting the loop.
...
| --- | --- | --- |
| **First-timer, one pool** | "I've never run a bot", "just the STX pool for now", names one wallet, no addresses yet | Steps 1 to 9 in full for that one pool, each step explained before it runs, the wallet's own taps and help link at Step 3. The other pool can be added later with a second account; it is not set up unasked. |
| **Both pools, addresses in hand** | Two `SP` addresses pasted, "set up both", knows their wallet | Step 1 is answered. Step 3 is only the account numbers. Everything from Step 3 on runs twice, once per pool, each with its own env file, key, enrollment and terminal. |
...
flowchart LR
  ...
  B --> C[3 One account per pool]
  C --> D[4 Fund it]
  D --> E[5 Key into env file]
  E --> F[6 Dry run]
  F --> G[7 Enroll]
  G --> H[8 Go live]
  H --> I[9 Watch]
...
The owner acts at 3, 4, 5 and 7; the agent runs everything else and checks after each owner step.
...
**Step 3. One account per pool (owner).** Ask which wallet, then give that wallet's steps and its own help page. The account must hold nothing but what the bot will use. Note its position in the account list: that is the account number Step 5 needs.
...
Then the agent creates the pool's env file from the repo's example and writes the address and a kill-switch path into it:

```bash
cd "$MM_REPO" && cp -n .env.<pool>.example .env.<pool> && chmod 600 .env.<pool>
sed -i '' 's/^SIGNER_ADDRESS=.*/SIGNER_ADDRESS=<SP address>/' .env.<pool>
echo "KILL_SWITCH_FILE=$MM_REPO/KILL_<pool>" >> .env.<pool>
grep -E '^(EXECUTION_MODE|SIGNER_ADDRESS|KILL_SWITCH_FILE)=' .env.<pool>
```

Done when the last command shows `EXECUTION_MODE=dry_run`, the address, and the kill-switch path.

**Step 4. Fund (owner).** The `sbtc` account needs sBTC and USDCx plus STX for gas. The `stx` account needs STX and USDCx; there STX is both gas and inventory. The bot keeps 5 STX back as a gas reserve and never deploys it, so fund above that. Arrive with both sides of the pair: a one-sided wallet makes the bot's first live action a de-risk sale, not a deploy. The agent reads the balances from the Hiro API and says back what is present and missing. Amounts are the owner's decision; the agent never picks a number.
...
The owner runs it on an offline machine or a terminal the agent is not reading, keeps the output on the clipboard, and clears shell history. The agent then opens the env file at the key line for the owner to paste, save and close, and does not read the file afterwards:
...
**Step 7. Enroll (owner).** Select the pool's account in the wallet. Open https://app.bitflow.finance/stack-sats, accept the terms, tap the Wallet button at the top right and, if another address shows, disconnect and reconnect choosing the wallet under Stacks Chain. Open Stack by Market Making and tap `Sign Transaction to Enroll`. The wallet shows a Sign Message prompt naming that account's address: check it, then Sign. It is a signature, not a transaction, so no gas. The agent then reads the campaign API:
...
For a second pool, repeat Steps 3 to 8 with its own account, env file and terminal. The `sbtc` loop serves metrics on port 8081 and the `stx` loop on 8082, so both can run side by side.
